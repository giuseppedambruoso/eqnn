...
def device_and_tensors():
    ...
    # Generate test images and apply L2 normalization
...
def test_p4m_equivariance_twirled(device_and_tensors, img_idx):
    ...
    qnn_node = create_qnn(
        device=DEVICE_NAME, p_err=P_ERR, reps=REPS, 
        equivariance=True, twirling=True, remove_cross_edge=False
    )
...
def test_xy_yy_unitary_compilation(device_and_tensors, img_idx):
    """Verifies that the compiled X/Y/YY unitary exactly mirrors the raw CNOT unitary across all layers."""
    torch_device, params, phi, test_images = device_and_tensors

    # Baseline: Original CNOT circuit (Eq=False, Twirl=False)
    qnn_cnot = create_qnn(device=DEVICE_NAME, p_err=P_ERR, reps=REPS, equivariance=False, twirling=False)
    
    # Target: Compiled X/Y/YY Unitary (Eq=True, Twirl=False)
    qnn_xy_yy = create_qnn(device=DEVICE_NAME, p_err=P_ERR, reps=REPS, equivariance=True, twirling=False)
    ...
    assert torch.allclose(out_cnot, out_xy_yy, atol=1e-3), \
        f"Compilation failed: X/Y/YY unitary does not match CNOT! Image Index {img_idx}"


@pytest.mark.parametrize("img_idx", range(NUM_IMAGES))
def test_cross_edge_removal_difference(device_and_tensors, img_idx):
    """Verifies that removing the 3->4 edge physically changes the unitary evolution."""
    torch_device, params, phi, test_images = device_and_tensors
    ...
    # Reduced Compiled Circuit (without 3->4 gate)
    ...
    actual_diff = abs((out_full - out_no_34).item())

    # The difference must be non-zero (since the unitary graph is topologically different).
    assert actual_diff > 1e-5, f"Edge removal failed: The outputs are identical!"
