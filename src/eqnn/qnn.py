...
def create_qnn(
    ...
    if equivariance and not twirling:
        ...
        for j_idx in range(7):
            # Dynamically generate QNodes to strictly isolate their quantum tapes
            # and remove 'j' from the Autograd signature, preventing NaN Jacobian collisions.
            def make_edge_qnode(j):
                @qml.qnode(dev, interface="torch", diff_method="parameter-shift")
                def _edge_readout(embedding_unitary: torch.Tensor, params: torch.Tensor) -> Any:
                ...
                return _edge_readout

            edge_qnodes.append(make_edge_qnode(j_idx))
        ...
        def qnn_forward(embedding_unitary: torch.Tensor, params: torch.Tensor, phi: torch.Tensor) -> Any:
            ...
            for j in range(7):
                ...
                if remove_cross_edge and j == 3:
                    continue
                total = total + edge_qnodes[j](embedding_unitary, params)
