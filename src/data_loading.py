...
class QuantumTestAugmentation:
    ...
    def __call__(self, img: torch.Tensor) -> torch.Tensor:
        ...
        if g_idx == 1: return TF.hflip(img)
        elif g_idx == 2: return TF.vflip(img)
        elif g_idx == 3: return TF.rotate(img, 180)
        elif g_idx == 4: return img.transpose(-1, -2)
        elif g_idx == 5: return TF.rotate(img, 90)
        elif g_idx == 6: return TF.rotate(img, 270)
        elif g_idx == 7: return img.transpose(-1, -2).flip(-1).flip(-2)
        return img

def load_mnist_data(
    batch_size: int, N: int, num_workers: int, img_size: int = 16, data_dir: str = "data",
    seed: int = 42, verbose: bool = False, augment_test: bool = False
    ...
    train_full = torchvision.datasets.MNIST(root=data_dir, train=True, download=True,
                                            transform=train_transform, target_transform=tar_transform)
    test_full = torchvision.datasets.MNIST(root=data_dir, train=False, download=True,
