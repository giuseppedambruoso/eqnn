...
def V_x(num_qubits: int) -> None:
    half = num_qubits // 2
    for i in range(half): qml.X(wires=i)

def V_y(num_qubits: int) -> None:
    half = num_qubits // 2
    for i in range(half, num_qubits): qml.X(wires=i)
...
def apply_group_element(g_idx: int, num_qubits: int) -> None:
    ...
    elif g_idx == 1: V_x(num_qubits) # Reflection X
    elif g_idx == 2: V_y(num_qubits) # Reflection Y
    elif g_idx == 3: # Reflection XY (180 Rotation)
        V_x(num_qubits); V_y(num_qubits)
    elif g_idx == 4: # Transpose (x-y swap)
        for i in range(half): qml.SWAP(wires=[i, i + half])
    elif g_idx == 5: # 90 Rotation
        V_x(num_qubits); [qml.SWAP(wires=[i, i + half]) for i in range(half)]
    elif g_idx == 6: # -90 Rotation
        V_y(num_qubits); [qml.SWAP(wires=[i, i + half]) for i in range(half)]
    elif g_idx == 7: # Anti-diagonal reflection
        V_x(num_qubits); V_y(num_qubits); [qml.SWAP(wires=[i, i + half]) for i in range(half)]
...
def create_qnn(
    ...
    def qnn_base(
        ...
        if equivariance and twirling:
            ...
            if p_err != 0:
                for i in range(num_qubits): qml.DepolarizingChannel(p_err, wires=i)
