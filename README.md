...
Parametri configurabili: `GENERAL.{seed,dev}`, `DATA.{N,dataset,img_size,augment_test,augment_train}`, `QNN.{num_qubits,reps,architecture,device,readout}`, `TRAINING.{epochs,learning_rate,patience,min_delta}` — vedi [src/config/config.yaml](src/config/config.yaml) per i default.
...
**Misura**: tutte e 9 le architetture misurano per default la stessa quantità — la media di Pauli-X su tutti i qubit. config1-config5 la ottengono applicando un Hadamard prima di misurare Z (H·Z·H = X); config6-config9 la misurano direttamente (`readout="avg_x"`, verificato numericamente identico a meno di 1e-8 al comportamento di config1-5 — vedi `tests/test_ansatz_builder.py::test_readout_avg_x_matches_sum_z_noiseless`). Per config6-config9 (solo) puoi scegliere invece `QNN.readout=x0_xhalf`, la misura originale del paper di riferimento: 0.5·(X₀+X_{n/2}), cioè solo il primo e il qubit-centrale, non la media su tutti — ignorato per config1-config5, il cui schema di misura è fisso.
...
**Aumento dati in training**: `DATA.augment_train` sceglie una tra tre modalità (default `"none"`):
- `"none"`: nessun aumento dati, training set cachato come al solito.
- `"online"`: una trasformazione p4m casuale (ribaltamento/rotazione/trasposizione) diversa **ad ogni epoca** per ogni immagine — vera augmentation, agisce da regolarizzatore contro l'overfitting a un singolo orientamento. Attenzione: disabilita la cache degli embedding per il training set (che altrimenti verrebbe "congelata" sulla prima trasformazione casuale invece di variare epoca per epoca), quindi ogni epoca torna a ricalcolare gli embedding da zero — più lento, ma è l'unico modo per avere aumento dati che funzioni davvero.
- `"once"`: una trasformazione p4m casuale scelta **una sola volta per immagine**, poi cachata e riusata per tutte le epoche (come il test set aumentato) — resta veloce, ma il modello vede un solo orientamento per ogni immagine di training per tutto il run: non dà lo stesso beneficio di regolarizzazione di `"online"`, è più vicino ad allenarsi su un dataset diverso ma comunque fisso.

**Velocità**: il training usa sempre `diff_method="backprop"` (differenziazione automatica, non "parameter-shift") ed esegue ogni batch in un'unica chiamata vettorizzata al circuito ([src/train.py:execute_batch](src/train.py)) invece che immagine per immagine — anche 10-100x più veloce a seconda dell'architettura. L'unico costo: `backprop` richiede lo stato interno del simulatore e **non è utilizzabile su hardware quantistico reale** (che invece richiederebbe `parameter-shift`, ancora disponibile passando `diff_method="parameter-shift"` a `create_qnn`/`build_qnn_from_spec` per chi programma direttamente in Python, ma non esposto come override da riga di comando). Inoltre, [src/data_loading.py](src/data_loading.py) calcola l'embedding quantistico di ogni immagine **una sola volta** prima del training (non ad ogni epoca, a meno di `augment_train="online"`) — con N=640 questo passaggio richiede alcuni minuti una tantum invece di ripetersi (moltiplicato per il numero di epoche) ad ogni epoca. L'ottimizzatore è `Adam(betas=(0.5, 0.999))`, per coerenza con lo script di riferimento su cui si basano config6-config9.

**Sweep in parallelo (`hydra/launcher=joblib`)**: ogni job dello sweep è un processo separato; `main.py` fissa `torch.set_num_threads(1)` per ciascuno, altrimenti ogni processo proverebbe a usare tutti i core della macchina per le proprie operazioni interne, causando un rallentamento drastico quando più job girano insieme (osservato: ~100x più lento con soli 2 job in parallelo su una macchina a 14 core).

**Raggruppamento automatico su wandb**: se non imposti esplicitamente `WANDB_RUN_GROUP`, ogni run viene assegnato automaticamente a un gruppo derivato da tutti i parametri di `config.yaml` **tranne il seed** (architettura, N, epoche, reps, ecc.) — quindi più seed della stessa identica configurazione finiscono nello stesso gruppo, e wandb mostra automaticamente media e deviazione standard su ogni grafico per quel gruppo (basta attivare il raggruppamento per "Group" nella dashboard). Impostare manualmente `-e WANDB_RUN_GROUP=...` (come per confrontare architetture diverse sullo stesso grafico) sovrascrive questo comportamento automatico.
...
- `circuit.txt`/`circuit.png` — il disegno (testuale e visivo) del circuito effettivamente addestrato, coi valori finali dei parametri,
...
- **Schema di misura**: "somma Z" (equivalente a media delle X, usato da `config1`-`config5`), "media delle X" (stessa quantità, misurata direttamente — default per `config6`-`config9`), oppure "X0 + X_{n/2}" (alternativa disponibile per `config6`-`config9`).
...
- `build_qnn_from_spec(device, num_qubits, spec, twirled=False, readout="sum_z")` costruisce il QNode, restituendo `(qnn_forward, initial_params, resolved_spec)` — `resolved_spec` ha ogni init casuale già "risolto" in un valore fisso, necessario per poter ricostruire lo stesso identico circuito in seguito (es. da un checkpoint). `twirled=True` avvolge lo spec nello stesso twirling p4m esplicito usato da `config2`/`config4`/`config5`. `readout` sceglie lo schema di misura: `"sum_z"` (somma Z con cambio di base H, il default, usato da `config1`-`config5`), `"avg_x"` (media diretta delle X su tutti i qubit — matematicamente identica a `"sum_z"`, default per `config6`-`config9`), o `"x0_xhalf"` (0.5·(X₀+X_{n/2}), alternativa per `config6`-`config9`).
